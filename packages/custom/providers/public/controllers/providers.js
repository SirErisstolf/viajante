...
angular.module('mean.providers').controller('ProvidersController',
  function ($scope, $sce, $stateParams, $location, Global, Providers, MeanUser, Circles, MeanUpload, ColorPicker) {
    $scope.global = Global;

    $scope.provider = {};

    $scope.imagesBanner = [];
    $scope.imageBannerSrc = [];

    $scope.imagesContent = [];
    $scope.imagesContentSrc = [];

    $scope.imagesServices = [];
    $scope.imagesServicesSrc = [];

    $scope.imagesCabin = [];
    $scope.imagesCabinSrc = [];
    $scope.imagesCabinToSave = [];

    $scope.files = [];
    $scope.banner = "";
    $scope.bgColorPage = "white";
    $scope.isAdmin = MeanUser.isAdmin;
    var d = new Date();
    $scope.year = d.getFullYear();

    var getLimit = function (section) {
      var len = "files/public/" + MeanUser.user.username + "/" + section + "/"
      return len.length;
    }

    $scope.getFileName = function (filePath, section) {
      return filePath.substring(getLimit(section), filePath.length);
    }

    $scope.uploadBannerFileCallback = function (file) {
      if (file.type.indexOf('image') !== -1) {
        $scope.imagesBanner.push(file);
        $scope.imageBannerSrc.push(file.src.substr(1));
        $scope.banner = file.src.substr(1);
      }
      else {
        $scope.files.push(file);
      }
      document.getElementsByTagName("input").file_browse.disabled = true;

    };

    $scope.uploadContentFileCallback = function (file) {
      if (file.type.indexOf('image') !== -1) {
        $scope.imagesContent.push(file);
        $scope.imagesContentSrc.push(file.src.substr(1));
      }
      else {
        $scope.files.push(file);
      }
    ...
    $scope.uploadServicesFileCallback = function (file) {
      if (file.type.indexOf('image') !== -1) {
        $scope.imagesServices.push(file);
        $scope.imagesServicesSrc.push(file.src.substr(1));
      }
      else {
        $scope.files.push(file);
      }
    ...
    $scope.uploadCabinFileCallback = function (file, index) {

      if (file.type.indexOf('image') !== -1) {
        $scope.imagesCabinSrc[index] = [];
        $scope.imagesCabinSrc[index].push(file.src.substr(1));
      }
      else {
        $scope.files.push(file);
      }
    ...
    $scope.removeBanner = function () {
      $scope.imageBannerSrc = [];
      document.getElementsByTagName("input").file_browse.disabled = false;
    };
    ...
    $scope.create = function (isValid) {
      ...
      if (isValid) {
        ...
        for (var i = 0; i < cabins.length; i++) {
          cabins[i].image = $scope.imagesCabinSrc[i];
        ...
        $scope.provider.bannerImg = encodeURI($scope.banner);
        $scope.provider.contentImages = $scope.imagesContentSrc;
        $scope.provider.servicesImages = $scope.imagesServicesSrc;
        $scope.provider.bgColorPage = "#ffffff";
        $scope.provider.bgColorSection = $scope.bgColorSection;
        $scope.provider.textColor = $scope.textColor;
    ...
    $scope.remove = function (provider) {
      var confirmed = confirm("¿seguro quieres eliminar éste alojamiento?");
      if (!confirmed) {
        return;
      }
      if (provider) {
    ...
    $scope.find = function () {
      Providers.query(function (providers) {
        if (MeanUser.isAdmin) {
          ...
          $scope.providers = profileFilter(providers);
        }
      });

    };

    function profileFilter(providers) {
      var pivot = [];
      for (var i = 0; i < providers.length; i++) {
        if (providers[i].user.username == MeanUser.user.username) {
          pivot.push(providers[i])
        }
      }
      return pivot;
    }
    ...
    $scope.findOne = function () {
      ...
      Providers.get({
        ...
        //debugger;
        for (var i = 0; i < $scope.cabins.length; i++) {
          $scope.imagesCabinSrc[i] = [];
          $scope.imagesCabinSrc[i].push($scope.cabins[i].image);
        }
        ...
        $scope.imageBannerSrc = [provider.bannerImg];
        $scope.imagesContentSrc = provider.contentImages;
        $scope.imagesServicesSrc = provider.servicesImages;
        $scope.bgColorSection = provider.bgColorSection;
        $scope.textColor = provider.textColor;

        if (provider.bannerImg && typeof(document.getElementsByTagName("input").file_browse) !== "undefined") {
          document.getElementsByTagName("input").file_browse.disabled = true;
        }
    ...
    $scope.removeCabin = function (index) {
      ...
      $scope.imagesCabinSrc.splice(index, 1);
    };
    ...
    $scope.removeContentImage = function (index) {
      $scope.imagesContentSrc.splice(index, 1);
    };

    $scope.removeServicesImage = function (index) {
      $scope.imagesServicesSrc.splice(index, 1);
    };

    $scope.addNotification = function () {
      $scope.provider.notifications = $scope.provider.notifications || [];
      $scope.provider.notifications.unshift({
        visible: true,
        message: "",
        tipo: "success"
      });
    ...
    $scope.removeNotification = function (index) {
      $scope.provider.notifications.splice(index, 1);
    };
  ...
  ['$scope', '$sce', '$stateParams', '$location', 'Global', 'Providers', 'MeanUser', 'Circles', 'MeanUpload'
  ]);